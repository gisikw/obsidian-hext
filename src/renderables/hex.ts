...
export class Hex {
  ...
  render(size: number): string {
	const { bg, fg } = this.terrain && TERRAIN_COLORS[this.terrain]
    ...
    const icon = `<g transform="translate(${iconX}, ${iconY}) scale(${iconSize / 512})" style="fill: ${fg}">${iconSVG.replace(/fill="[^"]+"/,'')}</g>`;
